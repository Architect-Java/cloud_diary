...
public class PageUtil<T> {
    ...
    private Integer totalPages; // 总页数（总记录数/每页显示的数量；将参数先转换为浮点型，执行出发操作，向上取整）
    private Integer prePage; // 上一页（当前页-1，如果当前页-1小于1，则上一页为1）
    private Integer nextPage; // 下一页（当前页+1，如果当前页+1大于总页数，则下一页为总页数的值）

    private Integer startNavPage; // 导航开始页（当前页-5，如果当前页-5小于1，则导航开始页为1，此时导航结束数为导航开始数+9，如果导航开始数+9大于总页数，则导航结束数为总页数）
    ...
    public PageUtil(Integer pageNum, Integer pageSize, long totalCount) {
        ...
        // 总页数（总记录数/每页显示的数量；将参数先转换为浮点型，执行出发操作，向上取整）
        ...
        this.startNavPage = pageNum - 5; // 默认的导航开始页，当前页-5
