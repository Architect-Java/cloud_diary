...
public class UserService {
    ...
    public ResultInfo<User> userLogin(String userName, String password) {
        ...
        // 设置到resultInfo对象中
        resultInfo.setResult(reUser);

        // 1. 判断参数是否为空
        if (StrUtil.isBlank(userName) || StrUtil.isBlank(password)) {
            //如果为空，将ResultInfo对象的状态码和提示信息
        ...
        // 判断加密后的密码是否与数据库中的一致
        if (!password.equals(user.getPassword())) { // 如果密码不正确
            // 如果密码不正确，将ResultInfo对象的状态码和提示信息
            resultInfo.setCode(0);
