...
function deleteDom(typeId) {
    ...
    if (trLength == 2) {
        // 表示此时只有一条记录，删除整个表格对象，并设置提示内容
        $("#myTable").remove();
        // 设置提示信息（设置父元素div的id）
        $("#myDiv").html("<h4>暂未查询到类型数据！</h4>");
    } else {
...
$("#btn_submit").click(function () {
    ...
    // 3. 发送Ajax请求到后台，执行添加或修改功能，返回ResultInfo对象
    $.ajax({
        type: "post",
        url: "type",
        data: {
...
function updateDOM(typeId, typeName) {
    ...
    // 修改左侧类型分组导航栏的列表项
    // 1. 给左侧类型名称设置span标签，并指定id属性值，修改该span元素的文本值
